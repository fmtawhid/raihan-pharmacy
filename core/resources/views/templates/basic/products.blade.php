        ...
        <div class="container">
            ...
            <div class="product-shop">
                <aside class="category-sidebar">
                    <button type="button" class="close-sidebar text--secondary d-xl-none"><i
                    ...
                    <form action="" method="get" id="filterForm" class="product-filter-form">
                                ...
                                <div class="widget-body">
                                    <ul class="filter-category filter-overflow">
                                        @foreach ($categories as $item)
                                            <li>
                                                <a href="{{ route('product.by.category', $item->slug) }}" class=""
                                                    data-slug="{{ $item->slug }}"><i class="las la-angle-right"></i>
                                                    {{ __($item->name) }} </a>
                                            </li>
                                        @endforeach
                                    </ul>
                                ...
                                <div class="widget-body filter-overflow">
                                    @foreach ($brands as $item)
                                        <div class="widget-check-group brand-filter">
                                            <input type="checkbox" class="form-check-input brand-item"
                                                value="{{ $item->slug }}" name="brand[]" id="brand-{{ $loop->iteration }}"
                                                @checked(in_array($item->slug, $brand))>
                                ...
                                <div class="price-range d-flex flex-wrap mb-4">
                                    <div class="widget-check-group">
                                        <input type="checkbox" class="form-check-input" id="priceCheck"
                                            @checked($request->has('min_price') || $request->has('max_price'))>
                                    ...
                                    <div class="input-group">
                                        <input type="number" step="any" class="form-control" min="0"
                                            name="min_price" value="{{ getAmount($minPrice) }}" disabled>
                                        <input type="number" step="any" class="form-control" min="0"
                                            name="max_price" value="{{ getAmount($maxPrice) }}" disabled>
                                    </div>
                                </div>
                                ...
                                <div class="widget-body">
                                            ...
                                            @foreach ($attribute->values as $attributeValue)
                                                <li>
                                                    <label class="select-color"
                                                        for="attr-color-val-{{ $attributeValue->id }}"
                                                        style="color:#{{ $attributeValue->value }}; background-color:#{{ $attributeValue->value }}"
                                                        data-bs-title="{{ __($attributeValue->name) }}">
                                                        <input class="select-color__input attributes" type="checkbox"
                                                            name="attribute_values[]" data-type="{{ $attribute->type }}"
                                                            value="{{ $attributeValue->id }}"
                                                            id="attr-color-val-{{ $attributeValue->id }}"
                                                            @checked(in_array($attributeValue->id, $attributeValues))>
                                                    </label>
                                                </li>
                                            @endforeach
                                        </ul>
                                    @else
                                        <div class="product-size-filter">
                                            @foreach ($attribute->values as $attributeValue)
                                                <div class="widget-check-group">
                                                    <input class="form-check-input attributes" type="checkbox"
                                                        data-type="{{ $attribute->type }}" name="attribute_values[]"
                                                        value="{{ $attributeValue->id }}"
                                                        id="attr-value-{{ $attributeValue->id }}"
                                                        @checked(in_array($attributeValue->id, $attributeValues))>
                                                    <label class="form-check-label"
                                                        for="attr-value-{{ $attributeValue->id }}">
                        ...
                        {{-- Special Offers ------------------------------------------------ --}}
                        <div class="widget">
                            <h5 class="title">@lang('Special Offers')</h5>
                            <div class="widget-body">

                                @php
                                    // grab the current query‑string value once
                                    $discType = request()->discount_type;
                                @endphp

                                {{-- Discount Type (mutually exclusive check‑boxes) --}}
                                <p class="small mb-2 fw-semibold">@lang('Discount Type')</p>

                                <div class="widget-check-group">
                                    <input type="checkbox" class="form-check-input discount-type" id="discount-fixed"
                                        name="discount_type" value="{{ \App\Constants\Status::DISCOUNT_FIXED }}"
                                        @checked($discType == \App\Constants\Status::DISCOUNT_FIXED)>
                                    <label for="discount-fixed">@lang('Fixed')</label>
                                </div>

                                <div class="widget-check-group">
                                    <input type="checkbox" class="form-check-input discount-type" id="discount-percent"
                                        name="discount_type" value="{{ \App\Constants\Status::DISCOUNT_PERCENT }}"
                                        @checked($discType == \App\Constants\Status::DISCOUNT_PERCENT)>
                                    <label for="discount-percent">@lang('Percent')</label>
                                </div>

                                {{-- Discount range toggle & slider --}}
                                {{-- <div class="price-range d-flex flex-wrap mb-3 mt-3">
                                    <div class="widget-check-group">
                                        <input type="checkbox" class="form-check-input" id="discountCheck"
                                            @checked(request()->has('min_discount') || request()->has('max_discount'))>
                                        <label class="form-check-label" for="discountCheck">
                                            @lang('Discount Range')
                                        </label>
                                    </div>
                                    <div class="input-group">
                                        <input type="number" step="any" class="form-control" name="min_discount"
                                            min="0" value="{{ request()->min_discount ?? 0 }}" disabled>
                                        <input type="number" step="any" class="form-control" name="max_discount"
                                            min="0" value="{{ request()->max_discount ?? 0 }}" disabled>
                                    </div>
                                </div>
                            ...
                            <div class="widget-body">
                                <div class="product-filter__card">
                                    <ul class="list">
                                        <li>
                                            <div class="widget-check-group">
                                                <input class="form-check-input rating" name="rating" value="5"
                                                    type="radio" id="five-star" @checked($rating == 5)>
                                        ...
                                        <li>
                                            <div class="widget-check-group">
                                                <input class="form-check-input rating" name="rating" value="4"
                                                    type="radio" id="four-star" @checked($rating == 4)>
                                        ...
                                        <li>
                                            <div class="widget-check-group">
                                                <input class="form-check-input rating" name="rating" value="3"
                                                    type="radio" id="three-star" @checked($rating == 3)>
                                        ...
                                        <li>
                                            <div class="widget-check-group">
                                                <input class="form-check-input rating" name="rating" value="2"
                                                    type="radio" id="two-star" @checked($rating == 2)>
                                        ...
                                        <li>
                                            <div class="widget-check-group">
                                                <input class="form-check-input rating" name="rating" value="1"
                                                    type="radio" id="one-star" @checked($rating == 1)>
                                ...
                                <ul class="view-style d-flex gap-2  align-items-center">
                                    <li>
                                        <a href="javascript:void(0)" class="active view-grid-style"><i
                                                class="las la-th-large"></i></a>
                                    </li>
                                    <li>
                                        <a href="javascript:void(0)" class="view-list-style"><i
                                                class="las la-list-ul"></i></a>
                                    </li>
                                </ul>
                            </div>
...
@push('script')
    <script>
        (function($) {
            ...
            /*****************************************************************
             *  Discount Type   (keeps only ONE check‑box selected)
             *****************************************************************/
            $(document).on('change', '.discount-type', function() {
                if ($(this).prop('checked')) {
                    $('.discount-type').not(this).prop('checked', false);
                }
                syncDiscountSlider(); // adjust slider range
                fetchProducts();
            });
            ...
            /*****************************************************************
             * Discount Slider helpers (0‑100 for %, 0‑maxFixed for fixed)
             *****************************************************************/
            const maxFixed = {{ $maxFixed }}; // provided by controller

            function makeDiscountSlider(min, max) {
                // destroy if it exists first
                $('#discount-slider').slider('destroy').slider({
                    range: true,
                    min: min,
                    max: max,
                    values: [min, max],
                    slide: function(e, ui) {
                        $('input[name=min_discount]').val(ui.values[0]);
                        $('input[name=max_discount]').val(ui.values[1]);
                    },
                    change: fetchProducts
                });
            }

            /* decide which range to show based on the checked box */
            function syncDiscountSlider() {
                const isPercent = $('#discount-percent').prop('checked');
                const isFixed = $('#discount-fixed').prop('checked');

                if (!isPercent && !isFixed) { // neither checked
                    $('#discountCheck').prop('checked', false).trigger('change');
                    $('#discount-slider').slider('disable');
                    return;
                }

                // range limits
                let max = isPercent ? 100 : maxFixed;
                makeDiscountSlider(0, max);

                // auto‑enable the range toggle
                $('#discountCheck').prop('checked', true).trigger('change');
            }
            ...
            $('#discountCheck').on('change', function() {
                if ($(this).prop('checked')) {
                    $('#filterForm [name=min_discount], #filterForm [name=max_discount]')
                        .attr('disabled', false);
                    $('#discount-slider').slider('enable');
                } else {
                    $('#filterForm [name=min_discount], #filterForm [name=max_discount]')
                        .attr('disabled', true);
                    $('#discount-slider').slider('disable');
