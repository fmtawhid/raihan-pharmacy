<script>
            ...
            if (productId) {
                ...
                $.each(products, function(i, v) {
                    ...
                    if ($(v).parents('.product-wishlist').find('.wishlist-label').length) {
                        $(v).parents('.product-wishlist').find('.wishlist-label').text(
    ...
    // add to cart handler
    // add‑to‑cart & buy‑now handler  🔄  ⬇⬇  REPLACE THE OLD FUNCTION WITH THIS
    function addToCartClickHandler() {

        const btn = $(this);
        const isBuyNow = btn.hasClass('buy-now-btn'); // 🔑 detect Buy‑Now
        const productId = btn.data('id');
        const productType = btn.data('product_type');

        // collect variant IDs if this is inside the product‑details page
        const attributeValues = $('.attributeBtn.active')
    .map((i, el) => $(el).data('attribute').id)
    .toArray();
        
    // const attributeValues = btn
    // .parents('.product-details')
    // .find('.attributeBtn.active')
    // .map((i, el) => $(el).data('attribute').id)
    // .toArray();

        // guard: variants must be selected for variable product
        if (productType == 2 && attributeValues.length < 1) {
            notify('error', 'Please select variants');
            return;
        }

        // quantity (defaults to 1 if no input in the card)
        const quantity = btn.parent().find('[name=quantity]').val() || 1;
        ...
        const addUrl = "{{ route('cart.add', ':id') }}".replace(':id', productId);

        $.post(addUrl, payload).done(res => {

            notify(res.status, res.message);

            if (res.status !== 'success') return;

            /* ——— 1.  Buy‑Now: redirect straight to full cart ——— */
            if (isBuyNow) {
                window.location.href = "{{ route('cart.page') }}";
                return; // ⬅ stop here, no side‑cart update
            }

            /* ——— 2.  Normal Add‑to‑Cart: update side cart ——— */
            setPartialCart(res.partialCartData);
            setCartCount(res.cartItemCount);
            setCartSubtotal(res.cartSubtotal);
        });
    }
            ...
            success: function(response) {
                if (response.status) {
                    ...
                    if (data.cart_products > 0) {
                        $('.header-middle .cartItemCount').text(data.cart_products).removeClass(
                            'd-none');
                    } else {
