...
@else
    <div class="row gy-4">
        ...
        @php
            $attributeForImage = $product->attributes
                ->whereIn('type', [Status::ATTRIBUTE_TYPE_COLOR, Status::ATTRIBUTE_TYPE_IMAGE])
                ->first();
        @endphp
        ...
        <div class="col-lg-12">
            ...
            <div class="row gy-3 position-sticky">
                ...
                @foreach ($variants as $variant)
                    @php
                        $startDate = $variant->sale_starts_from
                            ? showDateTime($variant->sale_starts_from, 'Y-m-d h:i A')
                            : null;
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div
                                class="card-header variant-item d-flex align-items-center justify-content-between flex-wrap gap-2">
                                ...
                                <div class="d-flex align-items-center gap-3 variant-item-bottom">
                                    <div class="d-flex justify-content-between align-items-center gap-2">
                                        ...
                                        <x-toggle-switch name="variants[{{ $loop->index }}][is_published]"
                                            value="1" :checked="@$variant->is_published" class="publishVariant" />
                                    </div>

                                    <a class="text-muted text-decoration-underline expandVariantBtn"
                                        data-bs-toggle="collapse" href="#singleVariant{{ $loop->index }}"
                                        role="button" aria-expanded="false"
                                        aria-controls="singleVariant{{ $loop->index }}">
                                        @lang('Expand')
                                    </a>
                                </div>
                            </div>
                                                ...
                                                <div
                                                    ...
                                                    <div class="d-flex flex-wrap gap-4">
                                                        <div class="d-flex gap-2 flex-shrink-0">
                                                            <x-toggle-switch
                                                                name="variants[{{ $loop->index }}][manage_stock]"
                                                                value="1" :checked="@$variant->manage_stock" class="manageStock" />
                                                            <label>@lang('Manage Stock')</label>
                                                        </div>

                                                        <div class="d-flex gap-2 flex-shrink-0 trackInventoryWrapper">
                                                            <x-toggle-switch
                                                                name="variants[{{ $loop->index }}][track_inventory]"
                                                                value="1" :checked="@$variant->track_inventory"
                                                        ...
                                                        <div class="d-flex gap-2 flex-shrink-0 showStockWrapper">
                                                            <x-toggle-switch
                                                                name="variants[{{ $loop->index }}][show_stock]"
                                                                value="1" :checked="@$variant->show_stock" />
                                                ...
                                                <div class="row gy-4">
                                                    <div class="col-lg-6">
                                                        <div class="form-group stockQuantityWrapper">
                                                            ...
                                                            <input type="number"
                                                                name="variants[{{ $loop->index }}][in_stock]"
                                                                class="form-control"
                                                                value="{{ $variant->in_stock }}" />
                                                            @if (@$variant->track_inventory)
                                                                <a href="{{ route('admin.products.stock.log.variant', $variant->id) }}"
                                                                    class="text-muted text-decoration-underline">@lang('View Stock Log')</a>
                                                            @endif
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="col-lg-6">
                                                        <div class="form-group alertQuantityWrapper">
                                                            ...
                                                            <input type="number"
                                                                name="variants[{{ $loop->index }}][alert_quantity]"
                                                                class="form-control"
                                            ...
                                            <div class="col-xl-12">
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group">
                                                        <input type="text" class="form-control variant-sku-field"
                                                            data-variant-index="{{ $loop->index }}"
                                                            name="variants[{{ $loop->index }}][sku]"
                                                            value="{{ $variant->sku }}">

                                                        <button type="button"
                                                            class="input-group-text text-muted autoGeneratedSku"
                                                            data-variant-index="{{ $loop->index }}"
                                                            data-sku="{{ generateSKU($product, $variant->name) }}">
                                                            @lang('Generate')
                                                        </button>
                                                    </div>

                                                    {{-- stored PNG (edit)  OR  live preview (create) --}}
                                                    @if ($variant->barcode_path)
                                                        <img src="{{ asset('storage/' . $variant->barcode_path) }}"
                                                            alt="barcode" height="80" class="mt-2">
                                                    @else
                                                        <div id="variantBarcodePreview-{{ $loop->index }}"
                                                            class="mt-2"></div>
                                                    @endif
                                                </div>
                                                ...
                                                @if ($product->is_downloadable && $product->delivery_type == Status::DOWNLOAD_INSTANT)
                                                    <div class="form-group">
                                                        ...
                                                        <input type="file" class="form-control"
                                                            name="variants[{{ $loop->index }}][file]"
                                                            accept=".zip" />
                                                        @if (@$variant->digitalFile)
                                                            <div class="text-end">
                                                                <a
                                                                    href="{{ route('admin.products.digital.download', encrypt($variant->digitalFile->id)) }}">@lang('View File')</a>
                                                            </div>
                                                        @endif
                                                    </div>
                                                @endif
                                            ...
                                            <div class="col-12">
                                                ...
                                                <div class="row">
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <div class="input-group">
                                                                ...
                                                                <input type="number" step="any"
                                                                    class="form-control"
                                                                    name="variants[{{ $loop->index }}][regular_price]"
                                                    ...
                                                    <div class="col-xl-6">
                                                        <div class="form-group">
                                                            <label
                                                                for="salePrice{{ $loop->index }}">@lang('Sale Price')</label>
                                                            <div class="input-group">
                                                                <span
                                                                    class="input-group-text">@lang(gs('cur_sym'))</span>
                                                                <input type="number" class="form-control"
                                                                    step="any" id="salePrice{{ $loop->index }}"
                                                                    name="variants[{{ $loop->index }}][sale_price]"
                                                    ...
                                                    <div class="col-xl-6">
                                                        <div class="form-group">
                                                            ...
                                                            <input type="text"
                                                                name="variants[{{ $loop->index }}][sale_starts_from]"
                                                    ...
                                                    <div class="col-xl-6">
                                                        <div class="form-group">
                                                            ...
                                                            <input type="text"
                                                                name="variants[{{ $loop->index }}][sale_ends_at]"
                                                                class="form-control dateTime"
                                                                value="{{ old('sale_ends_at', $endDate) }}"
                                                                autocomplete="off">
                                                        </div>
                                                    </div>
                                                </div>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="d-flex gap-3 variant-item-wrapper">
                                            <div>
                                                ...
                                                <x-media-uploader-input id="mainImage-{{ $loop->index }}"
                                                    for="product" isSingle="1" :images="$mainImage"
                                                    inputName="variants[{{ $loop->index }}][main_image]" />

                                                <span class="text-muted text--small">@lang('Image Size')
                                                    <b>{{ getFileSize('product') }}</b> @lang('px')</b></small>
                                            </div>
                                            ...
                                            <div class="flex-grow-1">
                                                ...
                                                <x-media-uploader-input id="galleryImages-{{ $loop->index }}"
                                                    for="product" :images="$galleryImages"
                                                    inputName="variants[{{ $loop->index }}][gallery_images]" />
    ...
    @push('script')
        <script>
            (function($) {
                ...
                $('.variant-sku-field').on('keyup blur', function() {
                    const sku = $(this).val().trim();
                    const idx = $(this).data('variant-index');
                    const $preview = $('#variantBarcodePreview-' + idx);

                    if (!sku) return $preview.html('');

                    $.post('{{ route('admin.products.barcode.preview') }}', {
                        _token: '{{ csrf_token() }}',
                        sku
                    }, res => {
                        if (res.base64) {
                ...
                const generateSkuBtnClickHandler = function() {
                    ...
                    $input.val($(this).data('sku')).trigger('keyup'); // shows preview
                }
                ...
                const showStockFields = (manageStockSwitch) => {
                    ...
                    showOrHideStockFields(manageStock, variantItem.find(trackInventoryCheckBox).prop('checked'),
                        variantItem);
                }

                const showInventoryFields = function(trackInventorySwitch) {
                    showOrHideStockFields(true, trackInventorySwitch.prop('checked'), trackInventorySwitch.parents(
                        '.singleVariantItem'));
                }
