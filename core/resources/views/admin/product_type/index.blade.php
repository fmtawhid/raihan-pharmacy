                            ...
                            <tbody>
                                <!-- @forelse($templates as $template)
                                    <tr>
                                        ...
                                        <td>
                                            @if($admin->can('edit_product-types'))
                                            ...
                                            @endif
                                            @if($admin->can('delete_product-types'))
                                                <form action="{{ route('admin.product.type.delete', $template->id) }}" method="POST" class="d-inline-block" onsubmit="return confirm('@lang('Are you sure to delete this product type?')')">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                @forelse($templates as $template)
                                    <tr>
                                        <td>{{ __($template->name) }}</td>
                                        @if($admin->can('edit_product-types') || $admin->can('delete_product-types'))
                                        <td>
                                            @if($admin->can('edit_product-types'))
                                                <a href="{{ route('admin.product.type.edit', $template->id) }}" class="btn btn-sm btn-outline--primary editBtn">
                                            ...
                                            @if($admin->can('delete_product-types'))
                                                <button type="button" class="btn btn-sm btn-outline--danger confirmationBtn" 
                                                        data-action="{{ route('admin.product.type.delete', $template->id) }}" 
                                                        data-question="@lang('Are you sure to delete this product type?')">
...
@push('breadcrumb-plugins')
    <div class="d-flex gap-3">
        <x-search-form />
        @if($admin->can('add_product-types'))
