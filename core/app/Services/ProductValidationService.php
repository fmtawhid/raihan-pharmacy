...
class ProductValidationService
{
    ...
    public function productValidationRule(Request $request, Product $product)
    {
        ...
        $validationRule = [
            ...
            // Purchase / Batch (inline “Receive Stock” card)
            'batch_no'       => 'nullable|string|max:40',
            'purchaser_id' => 'nullable|purchaser_id_valid',
            'new_purchaser' => function ($attribute, $value, $fail) use ($request) {
                if ($request->purchaser_id === 'new' && (!is_string($value) || trim($value) === '')) {
                    $fail(__('The new purchaser must be a non-empty string.'));
                }
            },
