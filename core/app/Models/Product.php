...
class Product extends Model
{
    ...
    {
        /* create a base SKU on insert */
        static::creating(fn($p) => $p->sku ??= Sku::base());

        /* optional: trash old barcode when a product is deleted  */
        static::deleted(function ($p) {
            if ($p->barcode_path) {
                Storage::disk('public')->delete($p->barcode_path);
            }
        });
    }
    ...
    {
        return $this->batches()
            ->selectRaw('SUM(qty_received-qty_sold) as s')->value('s') ?? 0;
    }
    ...
    public function scopeHasRunningOffer($q)
    {
        $q->whereHas('activeOffer', fn($offer) => $offer->running());
    }
