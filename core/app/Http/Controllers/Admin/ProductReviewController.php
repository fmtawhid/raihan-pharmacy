...
class ProductReviewController extends Controller
{
    ...
    {
        $review  = ProductReview::where('id', $id)->withTrashed()->firstOrFail();
        $product = $review->product;

        if ($review->trashed()) {
            $newReview = ProductReview::where('user_id', $review->user_id)
    ...
    {
        $request->validate([
            'status' => 'required|in:approve,archive,decline',
        ]);
