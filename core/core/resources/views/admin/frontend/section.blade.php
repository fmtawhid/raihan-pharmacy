                                ...
                                @foreach ($section->content as $k => $item)
                                    ...
                                    @else
                                        @if ($k != 'images')
                                            ...
                                            @elseif ($k == 'selected_deals')
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    <label>{{ __(keyToTitle($k)) }}</label>
                                                    <select class="form-control select2" name="{{ $k }}[]" multiple="multiple" data-placeholder="@lang('Select Deals')">
                                                        @php
                                                            $deals = \App\Models\MultiExpressDeal::where('status', 'active')->get();
                                                            $selected = @$content->data_values->$k ?? [];
                                                            if(is_string($selected)) $selected = explode(',', $selected);
                                                        @endphp
                                                        @foreach($deals as $deal)
                                                            <option value="{{ $deal->id }}" @if(in_array($deal->id, $selected)) selected @endif>{{ $deal->title }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                                ...
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        ...
                                                        @php
                                                            ...
                                                            if(is_array($value)) {
                                                                $value = implode(',', $value); // array কে string এ convert
                                                            }
                                                        @endphp
                                                        <input type="text" class="form-control" name="{{ $k }}" value="{{ $value }}">
                                                    </div>
                                                </div>
                                            @endif
                                            @php
