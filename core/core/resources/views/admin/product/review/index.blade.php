                            ...
                            @forelse($reviews as $review)
                                <tr>
                                    ...
                                    <!-- <td>
                                        ...
                                        <form action="{{ route('admin.products.reviews.status', $review->id) }}" method="POST">
                                            @csrf
                                            <select name="status" onchange="this.form.submit()" class="form-select form-select-sm">
                                                <option value="approve" {{ $review->status === 'approve' ? 'selected' : '' }}>Approve</option>
                                                <option value="archive" {{ $review->status === 'archive' ? 'selected' : '' }}>Archive</option>
                                                <option value="decline" {{ $review->status === 'decline' ? 'selected' : '' }}>Decline</option>
                                            </select>
                                        </form>
                                    ...
                                    <td class="text-nowrap">
                                        <div class="d-flex flex-wrap gap-1 align-items-center">
                                            ...
                                            <button type="button"
                                                    ...
                                                    data-id="{{ $review->id }}"
                                                    data-user="{{ __($review->user->username ?? 'N/A') }}"
                                                    data-rating="{{ $review->rating ?? 0 }}"
                                                    data-review="{{ $review->review ?? '' }}"
                                                    data-user_link="{{ route('admin.users.detail', $review->user->id ?? 0) }}">
                                                <i class="la la-desktop"></i> @lang('View')
                                            </button>
                                            ...
                                            {{-- Status Dropdown --}}
                                            <form action="{{ route('admin.products.reviews.status', $review->id) }}"
                                                method="POST"
                                                ...
                                                <select name="status"
                                                    ...
                                                    <option value="approve" @selected($review->status === 'approve')>@lang('Approve')</option>
                                                    <option value="archive" @selected($review->status === 'archive')>@lang('Archive')</option>
                                                    <option value="decline" @selected($review->status === 'decline')>@lang('Decline')</option>
                                                </select>
                                            </form>

                                            {{-- Delete / Restore Button --}}
                                            <button type="button"
                                                    class="btn btn-sm btn-outline--{{ $review->trashed() ? 'success' : 'danger' }} confirmationBtn"
                                                    data-action="{{ route('admin.products.reviews.delete', $review->id) }}"
                                                    data-question="@lang($review->trashed() ? 'Are you sure to restore this review?' : 'Are you sure to delete this review?')"
                                                    data-type="{{ $review->trashed() ? 'restore' : 'delete' }}"
                                                    data-id="{{ $review->id }}">
                                                <i class="la la-{{ $review->trashed() ? 'redo' : 'trash' }}"></i>
                                                @lang($review->trashed() ? 'Restore' : 'Delete')
                                            </button>
