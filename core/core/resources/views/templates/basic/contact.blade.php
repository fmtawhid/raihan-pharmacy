        ...
        <div class="container">
            <div class="row g-4 gy-4 @if($elements->count() || @$content->data_values->address_heading || @$content->data_values->description) justify-content-xl-between flex-wrap-reverse @else justify-content-center @endif">
                ...
                <div class="col-lg-6 col-xxl-8 col-xl-7 ps-xxl-5">
                    ...
                    <div class="contact-form">
                        <form 
                            action="{{ auth()->check() ? route('contact.submit') : '#' }}" 
                            method="post"
                            @if(!auth()->check()) onsubmit="event.preventDefault(); window.location='{{ route('user.login') }}';" @endif
                        >
                            @csrf
                            <div class="mb-3">
                                <label class="form--label" for="fullname">@lang('Full Name')</label>
                                <input type="text" class="form-control form--control" id="fullname" name="name"
                                    value="{{ old('name', auth()->check() ? auth()->user()->fullname : '') }}"
                                    @if(!auth()->check()) disabled @else readonly @endif required>
                            </div>
                            <div class="mb-3">
                                <label class="form--label" for="email">@lang('Email')</label>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control form--control" id="subject" name="subject"
                                    value="{{ old('subject') }}" @if(!auth()->check()) disabled @endif required>
                            </div>
                            <div class="mb-3">
                                <label class="form--label" for="message">@lang('Message')</label>
                                <textarea class="form-control form--control" name="message" rows="3" id="message"
                                    @if(!auth()->check()) disabled @endif required>{{ old('message') }}</textarea>
                            </div>
                            <x-captcha />
                            <div class="mb-3">
                                <button class="btn btn--base h-45" type="submit">
                                    {{ auth()->check() ? __('Send Your Message') : __('Send message after login') }}
                                </button>
                            </div>
                        </form>
                    </div>
